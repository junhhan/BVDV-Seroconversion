...
void _Create_animal_list(int n_farms, int **farm_info, int **group_size, animal_node *animal_list[]);
int ***_Status_bvdv(int **farm_info, animal_node *animal_list[]);
void _Seed_infection(int n_farms, int **farm_info, int **group_size, int ***bvdv, double *mu, double *rho);
...
//---------------------------------------------------------------------------------------------------
// MAIN FUNCTION
void bvdv_simulation(double *beta, double *rho, double *mu, int *tau, int *n_hfr, int *init_age, int *n_sample, 
		     int *f_psm, int *s_psm, int *mate_period, int *d_test01, int *d_test02, 
		     int *n_sim_pos01, int *n_sim_pos02, int *seed) {
	...
	// Pointer pointing BVDV status of all heifers
	int ***bvdv;	
	...
	for (i= 0; i< n_farms; i++) {
		...
		farm_info[i][3]= (*mate_period);// Mating period
	...
	_Create_animal_list(n_farms, farm_info, group_size, animal_list);
	bvdv= _Status_bvdv(farm_info, animal_list);
	...
	for(day= 1; day <= (*d_test02); day++) {
		...
		if (day == (*tau)) {
		    // Introduce BVDV
		    _Seed_infection(n_farms, farm_info, group_size, bvdv, mu, rho);
		}
...
// Random n sampling between two numbers
int *_Random_sample(int min, int max, int n) {
	int i, j, tmp, x= max - min + 1;

	int *array;
	array= calloc(x, sizeof(int));
	int *report;
	report= calloc(n, sizeof(int));
...
// Create a pointer pointing BVDV status of all heifers
int ***_Status_bvdv(int **farm_info, animal_node *animal_list[]) {
	int i, j;
	int ***bvdv;
	bvdv= calloc(n_farms, sizeof(int **));

	for (i= 0; i< n_farms; i++) {
		bvdv[i]= calloc(farm_info[i][0], sizeof(int *));
	}	
	...
	for (i= 0; i< n_farms; i++) {
		...
		while (current_node != NULL) {
			if (current_node->demo_group == 99999 || current_node->demo_group == -99999) {
				...
				bvdv[i][j]= &(current_node->inf_stat);
				j +=1;
				previous_node= current_node;
				current_node= current_node->next_node;
			}
...
void _Seed_infection(int n_farms, int **farm_info, int **group_size, int ***bvdv, double *mu, double *rho) {
	...
	int i, j, n_hfr, n_pi, n_r;
	int *sample;
	
	for (i= 0; i< n_farms; i++) {
		n_hfr= farm_info[i][0];
		n_r= (int)round((double)n_hfr * (*mu));
		n_pi= (int)round((double)(n_hfr - n_r) * (*rho));

		sample= _Random_sample(0, n_hfr - 1, n_r + n_pi);
		
		for (j= 0; j< n_r + n_pi; j++) {
			_Change_group_size(group_size, i, 0, 0, -1);  
			if (j < n_r) {
				(*bvdv[i][sample[j]])= 2;
				_Change_group_size(group_size, i, 0, 2, 1);  
			} else {
				(*bvdv[i][sample[j]])= 4;
				_Change_group_size(group_size, i, 0, 4, 1);  
			}
		}
	}
	free(sample);
